                ...
                data.stories.forEach(story => {
                    list.innerHTML += `
                        <div class="card_carro">
                            ...
                            <div class="description">${story.content}</div>
...
function saveComment() {
    ...
    const date = new Date().toISOString();  // Data atual para o comentário
    const last_upd = new Date().toISOString();  // Data de atualização também é a data atual

    // Verificar se os campos estão preenchidos
    if (!comment || !name || !mytology) {
    ...
    const newComment = {
        ...
        name: name,
        mytology: mytology  // Enviando o ID da mitologia
    };
    ...
    .then(data => {
        if (data.success) {
            ...
            document.getElementById('comment').value = '';  // Limpar campo de comentário
            document.getElementById('name').value = '';  // Limpar campo de nome
            // Atualizar lista de comentários, se necessário
